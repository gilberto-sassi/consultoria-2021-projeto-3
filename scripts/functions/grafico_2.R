...
#' @param variavel_fill a character (or factor) variable
...
#' 
#' @usage grafico_2(df, variavel_x, variavel_fill, rotulo_x, rotulo_y)
#' 
#' @examples
#' df <- tibble(x = sample(letters[1:5], 100, replace = T),
#'              y = sample(letters[6:10], 100, replace = T))
#' grafico_2(df, x, y, "x", "y")
grafico_2 <- function(df, variavel_x, variavel_fill, rotulo_x, rotulo_y = "", base = "") {
